...
import { MapPin, Briefcase, TrendingUp, Plus } from "lucide-react";
...
const MOCK_OPPORTUNITIES = [
  {
    id: "1",
    title: "招聘高级法律顾问",
    description: "知名互联网公司招聘高级法律顾问，负责公司日常法律事务处理",
  ...
  },
  {
    id: "2",
    title: "合同审查项目外包",
    description: "需要将一批标准合同进行审查分类，预计500份",
  ...
  },
  {
    id: "3",
    title: "法律顾问长期合作",
    description: "寻找长期法律顾问合作，处理企业日常法律咨询",
    opportunity_type: "collaboration",
    location: "深圳",
    publisher_name: "某创业公司",
    budget: 50000,
    is_featured: true,
    view_count: 890,
...
export default function OpportunityTypePage({ params }: PageProps) {
  ...
  }, [typeSlug, selectedLocation, page]);
  ...
  const fetchOpportunities = async () => {
    ...
    try {
      ...
      setOpportunities(
        (MOCK_OPPORTUNITIES.filter((o) => o.opportunity_type === typeSlug) as OpportunityRow[]) ?? []
      );
    } finally {
      setLoading(false);
    }
  ...
  const typeLabel = CATEGORIES.find((c) => c.value === typeSlug)?.label || typeSlug;
  ...
  return (
    <div className="min-h-screen bg-[rgba(212,165,116,0.08)]">
      <PageHeader
        title={`${typeLabel}机会`}
        description={`浏览所有${typeLabel}类型的合作机会`}
          ...
          filters={[
            {
              ...
              value: typeSlug,
              onChange: (value) => {
                if (value) window.location.href = `/opportunities/type/${value}`;
              },
            },
            {
              label: "地区",
      ...
      <section className="mx-auto max-w-6xl px-6 py-6 lg:px-8">
        {loading ? (
          ...
          <div className="rounded-2xl border border-[rgba(212,165,116,0.25)] marketing-cream-card-surface p-12 text-center">
            ...
            <h3 className="mt-4 text-lg font-semibold text-[#2C2416]">暂无{typeLabel}机会</h3>
                ...
                >
                  <div className="flex-1">
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-4 text-sm text-[#5D4E3A]">
                      ...
                      <span>{opp.view_count || 0} 浏览 · {opp.application_count || 0} 投递</span>
                    </div>
                  </div>
                </Link>
              ))}
