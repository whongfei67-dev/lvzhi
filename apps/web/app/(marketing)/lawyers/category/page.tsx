...
import { Users, ArrowRight, Scale, Star, MapPin } from 'lucide-react'
...
export default function LawyersCategoryPage() {
  return (
        ...
        <div className="mx-auto max-w-6xl px-6 py-16 lg:px-8 w-full">
          <div className="page-header-content animate-fade-in-up">
            <div className="inline-flex items-center gap-2 mb-4 px-4 py-1.5 rounded-full bg-[rgba(212,165,116,0.25)]">
              ...
              <span className="text-sm font-medium text-[#D4A574]">律师分类</span>
            </div>
            <h1 className="page-title">{preferTradForKeShiLu("认证律师")}</h1>
            <p className="page-desc">{preferTradForKeShiLu("320+ 位认证律师，覆盖各类法律领域")}</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-16">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {LAWYER_CATEGORIES.map((cat, idx) => (
              <Link
                key={cat.name}
                href={`/lawyers?domain=${encodeURIComponent(cat.name)}`}
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-[#9A8B78]">{cat.count} 位律师</p>
                  </div>
